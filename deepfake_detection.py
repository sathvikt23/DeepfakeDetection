...
def analyze_frame(frame):
    ...
    try:
        ...
        result = E.final_imageOnly(frame_image)
    ...
    try:
        ...
        if file_extension in ["mp4", "avi", "mov"]:
            ...
            for i, frame in enumerate(frames):
                result, frame_image = analyze_frame(frame)
                frame_results.append(result)
                progress = (i + 1) / (frame_count * 2)  # Account for audio processing
                progress_bar.progress(progress)
                if result == "Deepfake":
                    #data+=f"Potential Deepfake - Frame {i+1} result {result}\n"
                    st.image(frame_image, caption=f'Potential Deepfake - Frame {i+1}', use_column_width=True)
                    st.write(f"Frame {i+1}: {result}")
            ...
            if audio_model:
                ...
                try:
                    ...
                    if result == 0:
                        #data+="DEEPFAKE AUDIO DETECTED\nThe audio appears to be artificially generated"
            ...
            result = E.final_imageOnly(image)
